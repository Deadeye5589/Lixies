...
class Lixie{
	public:
		...
		void write_flip(char* input, uint16_t flip_time = 100, uint8_t flip_speed = 10);
		void write_flip(uint32_t input, uint16_t flip_time = 100, uint8_t flip_speed = 10);
		
		void write_fade(char* input, uint16_t fade_time = 250);
	...
	private:
		...
		static constexpr byte Addresses[10] = {3, 2, 4, 0, 8, 6, 5, 7, 9, 1};
		const static uint8_t LEDsPerDigit = 10;
		...
		CRGB nixie_col_ion = CRGB(255,70,25);
		CRGB nixie_col_aura = CRGB(45,85,255);
};
