...
const uint8_t leds_per_digit = 30;
...
uint16_t led_to_x_pos(uint16_t led){
  uint8_t led_digit_pos = x_offsets[led%30];
  ...
  return max_x_pos - (led_digit_pos + (complete_digits*14));
}
...
Lixie_II::Lixie_II(const uint8_t pin, uint8_t number_of_digits){
  ...
  max_x_pos = (number_of_digits * 14)-1;
...
void Lixie_II::begin(){
  max_power(5,9999); // Default for the safety of your PC USB
  start_animation();
}
...
void Lixie_II::streak(CRGB col, float pos, uint8_t blur){
  float pos_whole = pos*n_digits*14; // 6 X-positions in a single display
