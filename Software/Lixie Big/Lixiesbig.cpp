...
void Lixie::begin() {
	max_power(5,9999); // 5V, 1000mA DEFAULT
...
void Lixie::write_flip(char* input, uint16_t flip_time, uint8_t flip_speed){
	char temp[20] = "";
	byte index = 0;
	for(byte i = 0; i < 20; i++){
		if(char_is_number(input[i])){
	...
	uint32_t output = atol(temp);
	write_flip(output, flip_time, flip_speed);
}
...
void Lixie::write_fade(char* input, uint16_t fade_time){
	char temp[20] = "";
	byte index = 0;
	for(byte i = 0; i < 20; i++){
		if(char_is_number(input[i])){
...
void Lixie::write_digit(byte input, byte index, bool show_change){
  ...
  uint16_t L3 = start+Addresses[input] + 20;
...
void Lixie::push_digit(byte number) {
  ...
  uint16_t L3 = Addresses[number] + 20;
...
void Lixie::print_current() const{
	...
	static const uint8_t readdress[10] = {3, 2, 4, 0, 7, 6, 5, 8, 9, 1};
